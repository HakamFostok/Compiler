...
public class AubCompiler : ICompiler
{
    ...
    {
        get => locals.Gfile;
        set => locals.Gfile = value;
    }

    private TFile currentFile
    {
        get => locals.CurrFile;
        set => locals.CurrFile = value;
    }
    ...
    private static void ReadKeyWords()
    {
        ...
        while (!reader.EndOfStream)
        {
            ...
            TSymbol symbol = new() { Name = key, Next = null, UL = (TypeSymbol)counter };
            counter++;
    ...
    public void CompileMainProgram(string fileName)
    {
        ...
        using FileStream writer = new(dir, FileMode.Create);
        formatter.Serialize(writer, gProc);
    }

    private void MakeSyntaxError(string message) => throw new SyntaxErrorException(message, 0, 0, currentFile.Name);
}   // end class Compiler
