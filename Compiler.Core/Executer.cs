...
namespace Compiler.Core
{
    public partial class Executer
    {
            ...
            while (linst != null)
            {
                ...
                else if (linst.Ins is AssignInstruction)
                {
                    ...
                    Func<TExpression, TExpression, TExpression> a = (expValue, op) =>
                    {
                        ...
                        TExpression varExpression = new TExpression { UL = TypeSymbol.U_Var };
                ...
                else if (linst.Ins is ReadInstruction)
                {
                    ...
                    TExpression expAux = new TExpression { UL = TypeSymbol.U_Cst_Str, ValStr = result };
        ...
        private static void Assign(TVar v, int index, TExpression exp)
        {
            if (index != 0)
            {
                ...
                while (i < index)
                {
                    if (itemAux == null)
                    {
                        TItem newItem = new TItem { UL = TypeSymbol.U_UnKown };
        ...
        private static bool IsBoolean(TypeSymbol ul)
        {
            return (ul == TypeSymbol.U_True || ul == TypeSymbol.U_False);
        }

        private static bool IsNumber(TypeSymbol ul)
        {
            return (ul == TypeSymbol.U_Cst_Int || ul == TypeSymbol.U_Cst_Real);
        }
        ...
        private static void GetRuntimeError(string fullMessage, params string[] parameters)
        {
            MakeRuntimeError(string.Format(fullMessage, parameters));
        }

        private static void MakeRuntimeError(string fullMessage)
        {
            throw new RuntimeErrorException(fullMessage);
        }
    }
}